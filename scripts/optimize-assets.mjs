#!/usr/bin/env node
/**
 * Optimizes large SVG assets (with embedded raster) to WebP.
...
// SVG files with embedded raster to convert: [sourceFile, outputWebP]
const CONVERSIONS = [
...
async function convertSvgToWebp(sourceName, outputName) {
  ...
  return { outputName, inputSize, outputSize };
}
...
async function main() {
  ...
  for (const [source, output] of CONVERSIONS) {
    try {
      await convertSvgToWebp(source, output);
    } catch (e) {
