#!/usr/bin/env node
/**
 * Converts MP4 videos to WebM for better compression.
 */
...
async function convertVideo(input, output, crf) {
  ...
  execSync(
    `ffmpeg -i "${inputPath}" -c:v libvpx-vp9 -crf ${crf} -b:v 0 -c:a libopus -b:a 128k -threads 0 -y "${outputPath}"`,
    { stdio: 'inherit' }
  );
...
async function main() {
  console.log('Optimizing videos (MP4 → WebM)...\n');
